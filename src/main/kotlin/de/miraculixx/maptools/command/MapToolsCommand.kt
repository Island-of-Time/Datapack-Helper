...
class MapToolsCommand {
    private val command = commandTree("maptools") {
        ...
        anyExecutor { sender, _ ->
            sender.sendMessage(prefix + cmp("Version: ") + cmp(MapTools.INSTANCE.description.version, cHighlight) + if (tooling) cmp(" (Supporter Version)", cMark) else cmp(" (free version)", cError))
            if (tooling) {
